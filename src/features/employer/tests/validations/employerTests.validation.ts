...
const choiceQuestionTypes: TestQuestionType[] = [
  "single_choice",
  "multiple_choice",
  "true_false",
]
...
export function createTestQuestionOptionSchema(t: TFunction) {
  return z.object({
    ...
    option_text: z.string().trim().min(1, t("validation.optionTextRequired")).max(500, t("validation.optionTextMax")),
...
export function createTestQuestionSchema(t: TFunction) {
  return z.object({
    id: z.union([z.string(), z.number()]).optional(),
    question_text: z.string().trim().min(1, t("validation.questionTextRequired")).max(2000, t("validation.questionTextMax")),
    question_type: z.enum(validQuestionTypes),
    points: z.coerce.number().min(0, t("validation.pointsMin")).max(1000, t("validation.pointsMax")),
...
export function createEmployerTestSchema(t: TFunction) {
  return z
    .object({
      ...
      duration_minutes: z.coerce.number().int().min(1, t("validation.durationMin")).max(1440, t("validation.durationMax")),
    ...
    .refine((data) => !data.is_active || (data.questions?.reduce((sum, q) => sum + q.points, 0) ?? 0) > 0, {
...
export type TestQuestionOptionFormValues = z.infer<ReturnType<typeof createTestQuestionOptionSchema>>
