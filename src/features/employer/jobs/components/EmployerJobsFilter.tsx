...
interface EmployerJobsFilterProps {
  onApplyFilters: (values: EmployerJobFilterForm) => void
  onResetFilters: () => void
  isLoading: boolean
  initialFilters: Partial<EmployerJobFilterForm>
}

export default function EmployerJobsFilter({
  onApplyFilters,
  onResetFilters,
  isLoading,
  initialFilters,
}: EmployerJobsFilterProps) {
  const { t } = useTranslation("employerJobs")
  const fields: FilterField<EmployerJobFilterForm>[] = [
    {
      name: "work_mode",
  ...
  return (
    <CustomFilter<EmployerJobFilterForm>
      title={t("filters.title")}
      filters={fields}
      defaultValues={EMPLOYER_JOB_FILTER_DEFAULTS}
